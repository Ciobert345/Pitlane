...
export const useDataEngine = ({ updateState, updatePosition, updateCarData, connected }: Props) => {
	...
	const handleInitial = ({ CarDataZ: carZ, PositionZ: posZ, ...initial }: MessageInitial) => {
		...
		if (carZ) {
			try {
				...
				console.log(`[DataEngine] Inflated CarData. Entries: ${carData.Entries.length}`);
				if (carData.Entries.length > 0) {
					const firstEntryKeys = Object.keys(carData.Entries[0].Cars);
					console.log(`[DataEngine] First entry cars:`, firstEntryKeys);
				...
				console.error("Failed to inflate CarData", e);
			}
		}
		...
		if (posZ) {
			try {
				...
				console.error("Failed to inflate Position data", e);
			}
		}
	};
	...
	const handleUpdate = ({ CarDataZ: carZ, PositionZ: posZ, ...update }: MessageUpdate) => {
		...
		if (carZ) {
			try {
				const carData = inflate<CarData>(carZ);
				console.log(`[DataEngine] Update inflated CarData. Entries: ${carData.Entries.length}`);
				for (const entry of carData.Entries) {
					if (entry.Cars) {
						console.log(`[DataEngine] Pushing update for cars:`, Object.keys(entry.Cars));
					}
					carBuffer.pushTimed(entry.Cars, utcToLocalMs(entry.Utc));
				}
			} catch (e) {
				console.error("Failed to inflate CarData in update", e);
			}
		}
		...
		if (posZ) {
			try {
				...
				console.error("Failed to inflate Position data in update", e);
			}
		}
	};
