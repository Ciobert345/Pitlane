...
export const useDataStore = create<DataStore>((set) => ({
	...
	setCarsData: (carsData: CarsData | null) =>
		set((prev) => ({
			carsData: carsData ? { ...(prev.carsData ?? {}), ...carsData } : null,
		})),
	setPositions: (positions: Positions | null) =>
		set((prev) => ({
			positions: positions ? { ...(prev.positions ?? {}), ...positions } : null,
		})),
}));
