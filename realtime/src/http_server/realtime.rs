...
pub async fn sse_stream(
    ...
    let initial_state = match ctx.state_service.get_state_string().await {
        Ok(s) => {
            info!(size = s.len(), "Preparing initial SSE state");
            s
        }
        Err(e) => {
    ...
    let updates = BroadcastStream::new(rx)
        .filter_map(|result| async move {
            match result {
                Ok(msg) => {
                    info!(size = msg.len(), "Broadcasting SSE update");
                    Some(msg)
                },
                Err(e) => {
    ...
    let keep_alive = KeepAlive::new().interval(Duration::from_secs(5)).text("keep-alive-text");
